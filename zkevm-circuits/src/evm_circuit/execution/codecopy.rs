...
pub(crate) struct CodeCopyGadget<F> {
    ...
    /// Holds the memory address for the offset in code from where we read (checked with overflow).
...
impl<F: Field> ExecutionGadget<F> for CodeCopyGadget<F> {
    ...
    fn configure(cb: &mut ConstraintBuilder<F>) -> Self {
        ...
        let code_offset_lt_code_size = LtGadget::construct(cb, code_offset.expr(), code_size.expr());
        ...
        cb.condition(dst_memory_addr.has_length(), |cb| {
            cb.copy_table_lookup(
                ...
                select::expr(code_offset_lt_code_size.expr(), code_offset, code_size.expr()),
                code_size.expr(),
    ...
    fn assign_exec_step(
        ...
        let code_offset_within_range = self.code_offset_word.assign(region, offset, N_BYTES_U64, code_offset)?;

        let code = block
            .bytecodes
            .get(&call.code_hash)
            .expect("could not find current environment's bytecode");
        let code_size = code.bytes.len() as u64;
        self.code_size.assign(region, offset, Value::known(F::from(code_size)))?;
...
mod tests {
    ...
    fn test_ok(memory_offset: usize, code_offset: Word, size: usize, large: bool) {
        let mut code = bytecode! {};

        println!("gupeng - code = {:?}", code);
    ...
    fn codecopy_gadget_simple() {
        test_ok(0x00, 0x00.into(), 0x20, false);
        test_ok(0x20, 0x30.into(), 0x30, false);
        test_ok(0x10, 0x20.into(), 0x42, false);
    }

    #[test]
    fn codecopy_gadget_large() {
        test_ok(0x103, 0x102.into(), 0x101, true);
    }

    #[test]
    fn codecopy_gadget_code_offset_overflow() {
        test_ok(0x00, (u64::MAX - 1).into(), 1, false);
    }
}
